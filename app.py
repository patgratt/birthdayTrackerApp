...
def login():
    ...
    if request.method == 'POST':
        ...
        if not request.form.get('username'):
            return apology("You must provide a username to login!", 403)

        # Ensure password was submitted
        elif not request.form.get('password'):
            return apology("You must provide a password to login!", 403)
...
def register():
    ...
    if request.method == 'POST':
        ...
        if username == "" or password == "" or confirmation == "":
            return apology("One or more of the fields were left blank.", 999)
        if password != confirmation:
            return apology("The passwords that were entered did not match dude", 998)
        if len(db.execute('SELECT username FROM users WHERE username = ?', username)) > 0:
            return apology("The username that was entered has already been taken! Try a different username please!", 997)
...
def index():
    ...
    if request.method == 'GET':
        ...
        birthdays_rows = db.execute('SELECT name, month, day, id FROM birthdays WHERE user_id = ?', 
                                     current_user_id)
    ...
    if request.method == 'POST':
        ...
        db.execute('INSERT INTO birthdays (user_id, name, month, day) VALUES(?, ?, ?, ?)', 
                    current_user_id, name, month, day)
